...
/// Helper for conditionally applying view modifiers without duplicating view code.
extension View {
    @ViewBuilder
...
/// Drawer screen presenting a list of songs with:
/// - playlist context (all songs vs current playlist)
/// - search
/// - single and batch actions (delete / add to playlist)
/// - optional "windowed" list rendering for performance with large libraries
struct DrawerSongsListScreen: View {
    @Binding var isPresented: Bool
    @Binding var songs: [URL]
    @ObservedObject var playlistsManager: PlaylistsManager

    /// Identifier of the currently selected/playing song (used for UI highlighting).
    ...
    // Edit mode / multi-select / batch actions
    ...
    // Windowed list state (visible slice of rows)
    @State private var visibleRange: Range<Int>? = nil
    private let prefetchThreshold = 10 // Start expanding the window near list edges
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // Library info bar (song count + total duration)
                    counterBar()
                ...
                // Full-screen overlay for batch "Add to playlist"
            ...
            // Apply transitions only when animations are enabled
            .applyIf(drawerAnimEnabled) { view in
                view.transition(.move(edge: .bottom))
            }

            // Control show/hide animation based on user setting
            .animation(drawerAnimEnabled ? .easeInOut(duration: 0.26) : nil, value: isPresented)

            // Disable implicit animations for the subtree when animations are turned off
            .transaction { txn in
            ...
            // Overlay for single-item "Add to playlist"
            .overlay(
                Group {
            ...
            // Cleanup on dismiss (exit edit mode and persist playlists state)
            .onDisappear {
