...
/// Full-screen overlay for browsing and selecting playlists.
/// - Shows a searchable grid of user-created playlists (Favorites excluded).
/// - Supports creating and deleting playlists.
/// - Persists playlists on background/inactive transitions.
///
/// Note: This struct name is `PlaylistsOverlay`.
/// If other parts of the app use `PlayListOverlay`, rename either this view
/// or the call site to keep naming consistent.
struct PlaylistsOverlay: View {
    ...
    /// When a playlist is selected, this flag can be used by the parent to open the song list view.
    @Binding var showSongList: Bool

    /// Localization environment object.
    @EnvironmentObject var lang: Lang

    // MARK: - Local UI State

    /// Playlist selected for deletion confirmation.
    @State private var playlistToDelete: Playlist?

    /// Controls the delete confirmation alert.
    @State private var showDeleteAlert = false

    /// Controls the "create playlist" popup.
    @State private var showCreatePopup = false

    /// Local snapshot of playlists used for rendering & animation.
    @State private var localPlaylists: [Playlist] = []

    /// Search query for filtering playlists.
    @State private var searchText: String = ""

    /// Scene phase used to persist data on app backgrounding.
    @Environment(\.scenePhase) private var scenePhase

    // MARK: - Computed Properties

    /// Favorites playlist ID (excluded from the "user playlists" grid).
    private var favoritesID: UUID? { playlistsManager.favoritesPlaylist?.id }

    /// User-created playlists only (Favorites filtered out).
    private var userPlaylistsOnly: [Playlist] {
        let favID = favoritesID
        return localPlaylists.filter { $0.id != favID }
    }

    /// Search-filtered list of playlists.
    ...
    // MARK: - Background / Styling (split into subviews for compiler performance)
        ...
        HStack(spacing: 10) {
            ...
            HStack(spacing: 8) {
                ...
                TextField(lang.t("searchSong"), text: $searchText)
                    ...
                    // Prevent focus/typing while popup is visible
            ...
            // Create playlist button (hidden when limit reached)
            if userPlaylistsOnly.count < PlaylistsManager.maxPlaylists {
                Button {
                    // Dismiss keyboard/focus (especially search field) before showing popup
                    endEditing()
    ...
    var body: some View {
        ZStack {
            ...
            // Disable interactions behind popup
            .disabled(showCreatePopup)
            .zIndex(1)

            // MARK: - Create Playlist Popup
            ...
            if showCreatePopup {
                ...
                CreatePlaylistPopup(isPresented: $showCreatePopup) { name in
                    // Dismiss keyboard before mutating the model
                    endEditing()
                    searchText = ""

                    // Avoid animating the model update inside the popup flow
                    withTransaction(Transaction(animation: nil)) {
                        if let _ = playlistsManager.createPlaylist(name: name) {
                            localPlaylists = playlistsManager.playlists
                        } else {
                            print("⚠️ Playlist not created (limit reached or duplicate name).")
                        }
                    }
                }
                .frame(maxWidth: 360)
...
/// Single playlist card used inside the grid.
/// Shows: icon, playlist name, track count, total duration, and delete action.
private struct PlaylistCard: View {
    ...
    /// Cached total duration for this playlist.
    ...
    var body: some View {
        Button(action: onSelect) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 10) {
                    ...
                    // Inline delete button (kept separate from context menu)
        ...
        // Recalculate total duration whenever playlist content changes
        .task(id: playlist.songs) {
            await calcTotal()
        }

        // Long-press context menu for delete (secondary affordance)
        .contextMenu {
    ...
    /// Computes and caches the total duration of all playlist tracks.
    @MainActor
