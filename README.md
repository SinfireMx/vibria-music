...
This project was built as a **learning and portfolio app**, with emphasis on audio handling, state management, persistence, and system integration on iOS.

---

## ✨ Features

- 🎵 **Offline playback of local audio files** (mp3, m4a, aac, wav, aiff, caf)
...
- 🔺 **Triangular navigation UI** with smooth rotation animations
...
- **MVVM-style structure**
  - `MusicPlayerViewModel`  
    - Manages `AVPlayer`
    - Handles playback state, progress tracking, queue logic
    - Integrates with `MPRemoteCommandCenter`
- **Managers layer** for focused responsibilities:
  - `PlaylistsManager` – playlists & favorites
  - `ProgressManager` – playback progress persistence
  - `BookmarkManager` – file access bookmarks

### Persistence
- Imported files: **security-scoped bookmarks** stored in `UserDefaults`
- Playlists: `Codable` → JSON in `UserDefaults`
- Playback progress: `UserDefaults` with entry rotation (memory safety)
...
- `TrueTriangleTabView`  
  Triangular tab system and rotation animation
- `ContentView`  
...
No analytics, no network requests, no third-party SDKs.  
...
- Search & sorting in large libraries
